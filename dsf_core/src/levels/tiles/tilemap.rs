...
impl TileMap {
    ...
    pub fn is_tile_def_key(&self, pos: &Pos) -> bool {
        matches!(
            self.tiles.get(&self.wrapped(pos)),
            Some(Tile::TileDefKey(_))
        )
    }
    ...
    pub fn remove_tile(&mut self, pos: &Pos) -> Option<Pos> {
        ...
        let actual_pos = self.get_actual_pos(&wrapped_pos);
    ...
    /// Delegate function
    fn wrapped(&self, pos: &Pos) -> Pos {
        self.world_bounds.wrapped(pos)
    }
}
