...
use crate::levels::tiles::objects::{Block, Tool};
...
pub fn pickup_system(
    ...
    mut query_player: Query<(&mut Player, Entity, &Steering, &Transform)>,
...
pub fn use_tool_system(
    mut commands: Commands,
    mut audio: EventWriter<SoundEvent>,
    mut query_player: Query<(&mut Player, &Steering)>,
    ...
    for (mut player, steering) in query_player.iter_mut() {
        ...
        let targeted_blocks = match player.equipped {
            Some(ToolType::BreakBlocksHorizontally(depth)) => {
                let player_is_not_too_far_away_from_wall = at_least_one_is_breakable(
                    &tiles_to_side(1, steering, &tile_map.world_bounds),
                    &tile_map,
                );
                if player_is_not_too_far_away_from_wall {
                    Some(tiles_to_side(depth, steering, &tile_map.world_bounds))
                } else {
                    None
                }
            }
            Some(ToolType::BreakBlocksBelow(depth)) => {
                Some(tiles_below(depth, steering, &tile_map.world_bounds))
            }
            _ => None,
        };
        if let Some(targeted_blocks) = targeted_blocks {
            ...
            if at_least_one_is_breakable && none_are_unbreakable {
                ...
                for (entity, block) in query_blocks.iter() {
...
fn tiles_to_side(depth: u8, steering: &Steering, bounds: &WorldBounds) -> Vec<Pos> {
...
fn tiles_below(depth: u8, steering: &Steering, bounds: &WorldBounds) -> Vec<Pos> {
    let facing_offset = if steering.facing.x.is_positive() {
        steering.dimens.x - 1
    } else {
        0
